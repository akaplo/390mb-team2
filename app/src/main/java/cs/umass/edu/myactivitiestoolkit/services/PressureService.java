 ...
 * @see MobileIOClient
 */
public class PressureService extends SensorService implements SensorEventListener {

    /** Used during debugging to identify logs by class */
    private static final String TAG = PressureService.class.getName();
    ...
    public void onConnected() {
        ...
        mClient.registerMessageReceiver(new MessageReceiver(Constants.MHLClientFilter.ACTIVITY_DETECTED) {
            ...
            protected void onMessageReceived(JSONObject json) {
                Log.d(TAG, "Received pressure reading.");
                try {
                    JSONObject data = json.getJSONObject("data");
                    long timestamp = data.getLong("timestamp");
                    Log.d(TAG, "pressure reading occurred at " + timestamp + ".");
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_PRESSURE) {
            ...
            if (mLabel == ExerciseFragment.NO_LABEL) {
               // mClient.sendSensorReading(new PressureReading(mUserID, "MOBILE", "", timestamp_in_milliseconds, filterValues(event.values)));
            }
     ...
     */
    public void broadcastPressureReading(final long timestamp, final float[] pressureReadings) {
        Intent intent = new Intent();
        intent.putExtra(Constants.KEY.TIMESTAMP, timestamp);
        intent.putExtra(Constants.KEY.ACCELEROMETER_DATA, pressureReadings);
